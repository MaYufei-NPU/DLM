...
def get_named_beta_schedule(schedule_name, num_diffusion_timesteps):
	...
	elif schedule_name == 'sqrt':  # 基于αt生成βt
		return betas_for_alpha_bar(
			num_diffusion_timesteps,
...
def betas_for_alpha_bar(num_diffusion_timesteps, alpha_bar, max_beta=0.999):
	...
	:param num_diffusion_timesteps: the number of betas to produce.相当于文章中的T
	:param alpha_bar: a lambda that takes an argument t from 0 to 1 and
					  produces the cumulative product of (1-beta) up to that
					  part of the diffusion process.
	:param max_beta: the maximum beta to use; use values lower than 1 to
...
def get_x_start(x_start_mean, std):
	"""
...
class GaussianDiffusion:
	...
	def __init__(
		...
		assert (betas > 0).all() and (betas <= 1).all()  # βt必须在0~1之间
		...
		self.mapping_func = None  # scripts/train.py脚本中的get_mapping_func函数会对其进行替换
	...
	# 对前向过程进行采样，返回加噪后的信号x_t。这个x_t要作为model输入进而预测x_{t-1}
	...
	# 计算后向过程的均值和方差，目的是产生监督信号，即x_{t-1}的均值和方差
	def q_posterior_mean_variance(self, x_start, x_t, t):
		...
		:returns
		posterior_mean: True mean calculated in advance.
		posterior_variance: True variance calculated in advance.
		posterior_log_variance_clipped: True log variance calculated in advance.
		"""
	...
	def p_mean_variance2(
		...
		# 输出X0前对X0进行处理，使用自定义的denoise_fn或torch.clip(x, -1, 1)
		...
		if self.model_mean_type == ModelMeanType.PREVIOUS_X:
			...
			if self.model_mean_type == ModelMeanType.START_X:
				# 利用process_xstart处理后就可以得到近似X0
				pred_xstart = process_xstart(model_output)
			else:
				# case 2：模型预测噪声
				print('should go here')
				# 利用噪声和Xt预测X0，IDDPM公式9的变形
				pred_xstart = process_xstart(
	...
	def p_mean_variance(
		...
		Apply the model to get p(x_{t-1} | x_t), as well as a prediction of
		the initial x, x_0.
		However, it seems that this method is not utilized in method training_losses_e2e and
		the former one explicitly leverages model to predict x_start.

		:param model: the model, which takes a signal and a batch of timesteps
					  as input.
		:param x: the [N x C x ...] tensor at time t. Obtained from sampling.
		:param t: a 1-D Tensor of timesteps.
		:param clip_denoised: if True, clip the denoised signal into [-1, 1].
		:param denoised_fn: if not None, a function which applies to the
			x_start prediction before it is used to sample. Applies before
		...
		# 这一部分代码还在p_mean_variance里，根据预测结果（model_output）的种类，
		# 使用不同方法计算降噪过程的mean和variance
		"""以下是用模型的输出预测x_{t-1}的均值"""
		if self.model_mean_type == ModelMeanType.PREVIOUS_X:
			# case0: 直接预测Xt-1的期望值（均值）
			pred_xstart = process_xstart(
				self._predict_xstart_from_xprev(x_t=x, t=t, xprev=model_output)
			)  # IDDPM formula(11)
			model_mean = model_output
		elif self.model_mean_type in [ModelMeanType.START_X, ModelMeanType.EPSILON]:
			# case1: 预测X0，通过μ(xt, x0)公式变形间接计算
			if self.model_mean_type == ModelMeanType.START_X:
				pred_xstart = process_xstart(model_output)
			else:
				# case2：预测噪声ε，通过公式变形间接计算
				pred_xstart = process_xstart(
					self._predict_xstart_from_eps(x_t=x, t=t, eps=model_output)
				)
			model_mean, _, _ = self.q_posterior_mean_variance(
	...
	def p_sample(
		...
		"""
		p_sample_loop等会反复调用这个函数，训练时每隔一定步骤测试时会使用p采样
		Sample x_{t-1} from the model at the given timestep.

		:param top_p: maximum value permitted in sample, None by default
		...
		)  # 利用模型预测mean, var, log(var), x0
		# 反复矫正，直到tensor中的数据的绝对值都小于top_p
		if top_p is not None and top_p > 0:
			...
			while replace_mask.any():
				...
				replace_mask = th.abs(noise) > top_p
			assert (th.abs(noise) <= top_p).all().item()
	...
	def _vb_terms_bpd_e2e(
			...
			model_kwargs=None, denoised_fn=None,
	):
		"""
		Get a term for the variational lower-bound.
		...
		The resulting units are bits (rather than nats, as one might expect).
		This allows for comparison to other papers.
		:param input_ids: tokens from x0.
		:param get_logits: unknown param.
		:param x_start_mean: mean value of x0, necessary to sample x0.
		:param x_start_log_var: log variance of x0, necessary to sample x0.
		:param clip_denoised: True to clip x0 within [-1, 1].
		:param denoised_fn: used to process x0
	...
	def training_losses_emb(self, model, x_start, t, model_kwargs=None, noise=None):
		...
		:param x_start: the [N x C x ...] tensor of inputs. Datas extracted from dataset.
		...
		if noise is None:
			noise = th.randn_like(x_start)
		x_t = self.q_sample(x_start, t, noise=noise)  # 正向过程采样出的标签, 与t对应
	...
	def token_discrete_loss(self, x_t, get_logits, input_ids):
		"""
		:param x_t: 模型预测的输出
		:param get_logits: 对x_t进行处理的一个函数
		:param input_ids: 原句子中的token，标签
		:return: 交叉熵损失
		"""
		if self.model_arch == 'conv-unet' or self.model_arch == '1d-unet':
	...
	def x0_helper(self, model_output, x, t) -> dict:
		if self.model_mean_type == ModelMeanType.PREVIOUS_X:
	...
	def training_losses_e2e(self, model, t, model_kwargs=None, noise=None) -> dict:
		"""
		the last formula in Diffusion-LM Appendix E.
		Compute training losses for a single timestep.
		...
		:return: a dict with the key "loss" containing a tensor of shape [N].
				 Some mean or variance settings may also have other keys.
		"""
		assert 'input_ids' in model_kwargs, "input_ids not found in model_kwargs"
		# "input_ids" 是数据集中的数据的token
		input_ids = model_kwargs.pop('input_ids').to(t.device)
		x_start_mean = model.model.module.get_embeds(input_ids)  # word-embedding作为x0的均值
		if self.model_arch == 'conv-unet':
		...
		x_start = get_x_start(x_start_mean, std)  # 采样出的x0, 生成标签要用
		# print(x_start_mean.shape, x_start.shape)
		if noise is None:
			noise = th.randn_like(x_start)
		x_t = self.q_sample(x_start, t, noise=noise)  # re-parametrization trick.采样出xt，生成标签要用
		...
		if self.loss_type == LossType.E2E_KL:
		...
		elif self.loss_type == LossType.E2E_MSE:
			# print(x_t.shape)
			model_output = model(x_t, self._scale_timesteps(t), **model_kwargs)
			...
			target = {
				...
				ModelMeanType.START_X: x_start,
				ModelMeanType.EPSILON: noise,
			}[self.model_mean_type]  # 这就是生成的标签, e2e对应x_start

			assert model_output.shape == target.shape == x_start.shape
			# 计算模型预测的结果和target之间的MSE
			terms["mse"] = mean_flat((target - model_output) ** 2)
			# print( terms["mse"])
			# 模型预测的近似X0, x0_helper和p_mean_variance返回类似
			model_out_x_start = self.x0_helper(model_output, x_t, t)['pred_xstart']
			# t == 0 时需要修改loss
			t0_mask = (t == 0)
			# x_start_mean == word-emb
			t0_loss = mean_flat((x_start_mean - model_out_x_start) ** 2)
			terms["mse"] = th.where(t0_mask, t0_loss, terms["mse"])

			# tT_mask = (t == self.num_timesteps - 1)
			# 最后一个时间步的损失也需要修改，就是个定值
			out_mean, _, _ = self.q_mean_variance(x_start, th.LongTensor([self.num_timesteps - 1]).to(x_start.device))
			tT_loss = mean_flat(out_mean ** 2)

			# 文章中提到的rounding loss, 见附录E
	...
	def training_losses_e2e_simple(self, model, t, model_kwargs=None, noise=None):
		"""
		Compute training losses for a single timestep.
		...
		if noise is None:
			noise = th.randn_like(x_start)
		x_t = self.q_sample(x_start, t, noise=noise)  # re-parametrization trick.
		get_logits = model.model.module.get_logits
		...
		if self.loss_type == LossType.E2E_Simple_KL:
			...
			# tT_loss =  mean_flat(out_mean ** 2)
			# decoder_nll = self.token_discrete_loss(x_start, get_logits, input_ids) \
			# / model_output.size(-1) #DEBUG, only true for transformer model.
	...
	def calc_bpd_loop_e2e(self, model, x_start, clip_denoised=True, model_kwargs=None, denoised_fn=None):
		...
		x_start_log_var = 2 * th.log(std)
		# print(std)
		x_start = get_x_start(x_start_mean, std)
	...
	def calc_bpd_loop_emb(
		...
		:param denoised_fn: denoise function
		:param model: the model to evaluate loss on.
		:param x_start: the [N x C x ...] tensor of inputs.
		...
		vb, xstart_mse, mse = list(), list(), list()
