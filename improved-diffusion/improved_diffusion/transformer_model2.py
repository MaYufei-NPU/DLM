...
from .nn import (SiLU, checkpoint, linear, timestep_embedding, zero_module)
...
class TransformerNetModel2(nn.Module):
	...
	def __init__(
		...
		self.in_channels = in_channels  # word-emb dim
		self.model_channels = model_channels
		self.out_channels = out_channels
		self.num_res_blocks = num_res_blocks
		self.attention_resolutions = attention_resolutions
		...
		if training_mode == 'e2e':
			self.word_embedding = nn.Embedding(vocab_size, self.in_channels)  # (821, 16)
			...
			with th.no_grad():
				self.lm_head.weight = self.word_embedding.weight  # 让两个矩阵的参数一致
		...
		elif experiment_mode == 'lm':  # lm indicates language-model
	...
	def get_logits(self, hidden_repr):
		if self.logits_mode == 1:
			...
			dist = emb_norm + arr_norm.transpose(0, 1) \
				   - 2.0 * th.mm(self.lm_head.weight, text_emb_t)  # (vocab, d) x (d, bsz * seqlen)
			scores = th.sqrt(
				th.clamp(dist, 0.0, np.inf)
			).view(emb_norm.size(0), hidden_repr.size(0), hidden_repr.size(1))  # vocab, bsz*seqlen
