...
def load_data_text(
	...
	Each image is an N*C*H*W float tensor, and the kwargs dict contains zero or
	...
	if task_mode == 'roc' or task_mode == 'roc-aug':
		training_data, model = get_corpus_rocstory(
			data_args, model, image_size, padding_mode=padding_mode, split=split, load_vocab=load_vocab
		)
	...
	if data_args.modality in ['roc-aug', 'roc', 'book', 'yelp', 'commonGen', 'commonGen-aug'] \
...
def helper_tokenize_encode(sentence_lst, vocab_dict, model, seqlen, data_args, padding_mode):
	...
	with torch.no_grad():
		...
		if padding_mode == 'block':
			...
			# 把二维列表展开为一个一维列表，类似于tensor.flatten()和ndarray.flatten()
			...
			block_size = seqlen  # image_size ** 2
			total_length = (total_length // block_size) * block_size
...
def get_corpus_rocstory(
	...
	if data_args.experiment_mode == 'lm':
		if data_args.modality == 'roc':
		...
		elif data_args.modality == 'roc-aug':
			print('loading dataset from ROCStory')
			nlp = English()
			tokenizer = nlp.tokenizer
			sentence_lst = []
			if split == 'train':
				print('loading form the TRAIN set')
				path_lst = [
					f'{data_args.roc_train}/roc_train.json',
					r'diffusion_lm/improved-diffusion/diff_models/rocstories_gptj.txt'
				]
	...
	if data_args.experiment_mode == 'lm' \
		and data_args.modality in ['roc-aug', 'roc', 'yelp', 'commonGen', 'commonGen-aug'] \
		and data_args.cache_mode == 'no':
