...
class TrainLoop:
	...
	def forward_backward(self, batch, cond):
		zero_grad(self.model_params)  # 模型参数的梯度全设为0
		for i in range(0, batch.shape[0], self.microbatch):  # 步长==batch_size
			micro = batch[i: i + self.microbatch].to(dist_util.dev())
			micro_cond = {
				k: v[i: i + self.microbatch].to(dist_util.dev())
				for k, v in cond.items()
			...
			# print(micro_cond.keys())
			compute_losses = functools.partial(
				self.diffusion.training_losses,  # training_losses_e2e
				self.ddp_model,
				micro,
				t,
				model_kwargs=micro_cond,
	...
	def save(self):
		def save_checkpoint(rate_, params_):
			state_dict = self._master_params_to_state_dict(params_)
			if dist.get_rank() == 0:
				logger.log(f"saving model {rate_}...")
				if not rate_:
					filename = f"model{(self.step + self.resume_step):06d}.pt"
				else:
					filename = f"ema_{rate_}_{(self.step + self.resume_step):06d}.pt"
