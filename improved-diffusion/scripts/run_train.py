...
if __name__ == '__main__':
	...
	parser.add_argument('--image_size', type=int, default=8, help='')  # image_size是干啥的?
	...
	parser.add_argument('--seed', type=int, default=101, help='')  # old is 42(原本就有的注释)
	...
	# 三种loss function可选，如果没选就报错。但默认使用Lsimple
	...
	# Commandline第二行，python scripts/train.py 开始运行训练脚本
	...
	print(COMMANDLINE)
	if args.submit == 'no':
		# os.system把字符串转换成命令行指令
		os.system(COMMANDLINE)  # textattack/roberta-base-ag-news # textattack/roberta-base-imdb
	# #
	elif args.submit == 'yes':
		if args.use_big == 'no':
			full_command = \
				f"nlprun  -g 1 -n {Model_FILE} -x " \
				"jagupard10,jagupard11,jagupard12,jagupard13,jagupard14," \
				f"jagupard15,jagupard16,jagupard17,jagupard20 \'{COMMANDLINE}\'"
		elif True:
			full_command = \
				"nlprun  -g 1 -n {} -x " \
