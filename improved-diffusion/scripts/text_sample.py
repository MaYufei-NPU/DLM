...
from improved_diffusion.rounding import load_models, load_tokenizer, rounding_func
from improved_diffusion.script_util import (NUM_CLASSES, add_dict_to_argparser, args_to_dict,
											create_model_and_diffusion, model_and_diffusion_defaults)
from improved_diffusion.test_util import denoised_fn_round, get_weights
...
def main():
	...
	with open(config_path, 'rb') as f:
		training_args: dict = json.load(f)
	training_args['batch_size'] = args.batch_size  # 64
	...
	model2, tokenizer = load_models(
		args.modality, args.experiment, args.in_channel, os.path.split(args.model_path)[0]
	)  # 加载保存好的文件
	...
	all_images, all_labels = list(), list()
	print(args.num_samples)  # 50

	model3 = get_weights(model2, args)  # model2.weight.requires_grad = False
	while len(all_images) * args.batch_size < args.num_samples:  # 取50个数据
		model_kwargs = dict()
		if args.experiment_mode == 'conditional_gen':
			...
			if args.mbr_sample > 1:
				model_kwargs = {
					k: v.to(dist_util.dev()).repeat_interleave(args.mbr_sample, dim=0)
		...
		if args.model_arch == '1d-unet':
			...
			if args.mbr_sample > 1 and args.experiment_mode == 'conditional_gen':
				...
				sample_shape = (args.batch_size, args.image_size ** 2, args.in_channel)  # (50, 64, 16)
		print(sample_shape)
		sample = sample_fn(
			model,
			sample_shape,
	...
	if diffusion.training_mode.startswith('e2e'):
		...
		if args.model_arch == 'conv-unet':
			...
			reshaped_x_t = x_t
		logits = model.get_logits(reshaped_x_t)  # bsz, seqlen, vocab
		cands = th.topk(logits, k=1, dim=-1)  # 相当于argmax()
		sample = cands.indices
		tokenizer = load_tokenizer(args.modality, args.experiment, os.path.split(args.model_path)[0])
		for seq in sample:
			if isinstance(tokenizer, dict):
	...
	if args.verbose == 'yes':  # "yes"
		...
		if diffusion.training_mode.startswith('e2e'):
			...
			word_lst = rounding_func(
				args.experiment, arr, model, tokenizer, emb_scale_factor=args.emb_scale_factor
			)
